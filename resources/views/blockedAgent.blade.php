                ...
                @foreach ($data as $key => $agent)
                        <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                            ...
                            <td class="flex px-6 py-4 space-x-2 " >
                                <!-- {{$agent->user_id}} -->
                               <a href="{{ route('users.show',$agent->user_id)}}">  <button 
                                ...
                                <form action="{{ route('users.unblock', $agent->user_id) }}" method="put">
                                    @csrf
                                    <button type="submit"
                                ...
                                <form action="{{ route('users.delete', $agent->id) }}"  method="POST">
                                    @csrf
                                        @method('delete')
